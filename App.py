...
from flask import Flask, request, redirect, url_for, session, flash, abort, render_template_string, render_template
...
app = Flask(__name__)
# --- ALEMA HOMEPAGE (Abii folder + online counter + full playlist) ---------
import os
import uuid
from datetime import datetime, timedelta
from pathlib import Path
from flask import render_template_string, send_from_directory, session, jsonify

# Secret key (only if not already set elsewhere)
if not getattr(app, "secret_key", None):
    app.secret_key = os.environ.get("FLASK_SECRET_KEY", "change-me-in-prod")

# ---------- Resolve Abii media folder robustly ----------
BASE_DIR = Path(__file__).resolve().parent

# Let env override if you want an absolute path:
env_dir = os.environ.get("ALEMA_MEDIA_DIR")
candidates = [Path(env_dir)] if env_dir else []

# Typical locations (first existing wins)
candidates += [
    BASE_DIR / "Abii",                       # Abii beside app.py  <-- your case
    BASE_DIR / "jenni_new" / "Abii",
    BASE_DIR / "Jenni_new" / "Abii",
    BASE_DIR.parent / "jenni_new" / "Abii",
    BASE_DIR.parent / "Jenni_new" / "Abii",
]
...
if MEDIA_DIR is None:
    MEDIA_DIR = BASE_DIR / "Abii"           # create a sane default if missing
    MEDIA_DIR.mkdir(parents=True, exist_ok=True)

IMAGE_FOLDER = str(MEDIA_DIR)
LOGO_NAME = "db_lotto_hall.jpg"             # put this JPG in Abii

# ---------- Online / total visitors tracking ----------
ONLINE_WINDOW = timedelta(minutes=5)
VISITS_FILE = os.path.join(IMAGE_FOLDER, "_visits_count.txt")
ONLINE_USERS = {}

def _load_total_visits() -> int:
    try:
...
def _save_total_visits(n: int) -> None:
    try:
...
@app.before_request
def _track_user():
    """Track unique sessions and who is online now."""
    global TOTAL_VISITS
    if "vid" not in session:
        session["vid"] = uuid.uuid4().hex
        TOTAL_VISITS += 1
        _save_total_visits(TOTAL_VISITS)

    ONLINE_USERS[session["vid"]] = datetime.utcnow()

    # purge stale
    cutoff = datetime.utcnow() - ONLINE_WINDOW
    for sid, last in list(ONLINE_USERS.items()):
        if last < cutoff:
            ONLINE_USERS.pop(sid, None)


@app.before_request
def _require_disclaimer_before_login():
    """
    Force users to accept the disclaimer before reaching the login page.
    If they haven't accepted yet, redirect to /disclaimer and preserve the target URL.
    """
    if request.endpoint == "login" and not session.get("disclaimer_ok"):
        return redirect(url_for("disclaimer", next=request.url))

@app.route("/stats/online")
def stats_online():
    return jsonify(online=len(ONLINE_USERS), total=TOTAL_VISITS)

# ---------- Homepage ----------
@app.route("/")
def alema_home():
    """
    Public ALEMA landing page.
    Uses ONLY files from Abii: images show for a short time; videos play to end.
    """
    try:
    ...
    # Do not include the logo in the slideshow
    files = sorted(
        f for f in entries
        if f.lower().endswith((".jpg", ".jpeg", ".png", ".gif", ".webp"))
        and f.lower() != LOGO_NAME.lower()
    )
    videos = sorted(
        v for v in entries
        if v.lower().endswith((".mp4", ".webm", ".ogg"))
    )

    # One playlist (alphabetical)
    media_all = [{"kind": "image", "name": f} for f in files] + [{"kind": "video", "name": v} for v in videos]
    media_all.sort(key=lambda x: x["name"].lower())

    # Optional: exact captions per slide
    per_slide_caption = {
        # "cxk_01.jpg": "Hello, I am Christiana Xoryesuse Kove...",
        # "intro.mp4": "Intro video caption...",
    }

    # Defaults by owner (prefix 'cxk' => Christiana, else Abigail)
    christi_greeting = "Welcome, I am Christiana Xoryesuse Kove."
    christi_body = (
        "Why keep wasting data on loan apps downloads when afterwards you won’t be given the amount that "
        "will satisfy your needs and wants or even empower your business? Just that little amount you have "
        "is enough to start a new life with our lotto app. Subscribe to our weekly packages ranging from "
        "20gh for 5 games each day and 100gh for all 42 games a week. Winning guaranteed!"
    )
    default_christi_manual = f"{christi_greeting} {christi_body}"

    default_abigail = (
        "Welcome to the Divine Brain Lottery Forecast Centre. "
        "Don’t give up. Join the family that cares about your future. "
        "At Divine Brain Lotto, your miracle could be just one prediction away."
    )

    captions = {}
    for item in media_all:
        fname = item["name"]
        captions[fname] = per_slide_caption.get(
            fname,
            default_christi_manual if fname.lower().startswith("cxk") else default_abigail
        )
    ...
    <head>
      <meta charset="utf-8"/>
      <title>Divine Brain Lotto Slideshow + Video</title>
      <style>
        html, body { height: 100%; }
        body { margin:0; background:#2a1f14; height:100vh; display:flex; flex-direction:column; color:#fff; font-family:Georgia, serif; overflow:hidden; }
        .container { flex:1; display:flex; flex-direction:column; width:100%; height:100%; }
        .topbar { height:56px; display:flex; align-items:center; justify-content:space-between; padding:0 16px; background:#1b140e; border-bottom:2px solid #d4af37; }
        .brand { display:flex; align-items:center; gap:10px; font-weight:800; letter-spacing:.5px; }
        .brand img { width:42px; height:42px; border-radius:9px; box-shadow:0 2px 10px rgba(0,0,0,.4); }
        .brand span { color:#ffd700; font-size:1.05rem; }
        .stats { font-size:.95rem; color:#ffd700; }
        .pill { background:rgba(255,215,0,.15); border:1px solid #ffd700; padding:3px 8px; border-radius:999px; margin-left:8px; }

        .main { flex:1; display:flex; min-height:0; }
        .slideshow { flex:2; position:relative; display:flex; justify-content:center; align-items:center; background:#000; overflow:hidden; }
        .slide { max-width:100%; max-height:100%; border-radius:20px; box-shadow:0 8px 20px rgba(0,0,0,.7); opacity:0; position:absolute; transition:opacity 1s ease-in-out; object-fit:contain; z-index:1; }
        ...
        .number { position:absolute; top:-10%; font-size:26px; font-weight:700; color:gold; opacity:.8; text-shadow:0 0 8px rgba(255,215,0,.85); animation:fallTop var(--dur,14s) linear forwards, sway var(--swayDur,4s) ease-in-out infinite; }
        @keyframes fallTop { from{top:-10%;} to{top:110%;} }
        @keyframes sway { 0%{transform:translateX(calc(-1*var(--sx,20px)));} 50%{transform:translateX(var(--sx,20px));} 100%{transform:translateX(calc(-1*var(--sx,20px)));} }

        .watermark { position:absolute; bottom:10px; right:10px; opacity:.9; z-index:3; }
        .watermark img { width:90px; border-radius:8px; }
        ...
        .contacts { margin-top:10px; font-size:.92em; }
        .app-link { margin-top:14px; padding:10px 16px; background:gold; color:#000; border-radius:10px; font-weight:700; text-decoration:none; transition:background .3s; }
        .app-link:hover { background:#ffda47; }
        .typewriter { border-right:2px solid gold; animation:blink .8s infinite; }
        @keyframes blink { 50% { border-color:transparent; } }
        .flash { animation:flash .55s ease-in-out 3; }
        @keyframes flash { 0%,100%{opacity:1;} 50%{opacity:0;} }

        .bottom-bar { height:34px; background:#111; color:#ffd700; border-top:2px solid gold; display:flex; align-items:center; justify-content:center; position:relative; overflow:hidden; white-space:nowrap; font-size:.95em; }
        .credits-track { position:absolute; left:0; right:0; }
        .credits-text { display:inline-block; padding-left:100%; animation:scroll-left 45s linear infinite; }
        @keyframes scroll-left { 0%{transform:translateX(0);} 100%{transform:translateX(-100%);} }
        .hidden { display:none; }
        .disclaimer { color:#ffa2a2; text-align:center; padding-inline:8px; }

        .nav-buttons { position:absolute; left:0; right:0; bottom:10px; display:flex; justify-content:space-between; padding:0 12px; z-index:6; pointer-events:none; }
        .nav-buttons button { pointer-events:auto; border:none; background:rgba(0,0,0,.55); color:#fff; padding:8px 12px; border-radius:10px; font-weight:700; cursor:pointer; box-shadow:0 2px 8px rgba(0,0,0,.35); }
        .nav-buttons button:hover { background:rgba(0,0,0,.75); }
        .label-chip { position:absolute; top:10px; left:10px; z-index:6; background:rgba(0,0,0,.55); color:#fff; padding:6px 10px; border-radius:999px; font-weight:700; }

        @media(max-width:768px){ .main{flex-direction:column;} .slideshow{flex:none;height:64%;} .info{flex:none;height:36%;border-left:none;border-top:3px solid gold;} }
      </style>
    </head>
    <body>
      <div class="container">
        <div class="topbar">
          <div class="brand">
            ...
            <span>DIVINE BRAIN — Lotto Forecast Center</span>
          </div>
          <div class="stats">
            <span class="pill">Visitors online now: <strong id="onlineCount">{{ online_now }}</strong></span>
            <span class="pill">Total visits: <strong id="totalVisits">{{ total_visits }}</strong></span>
          </div>
        </div>
        ...
        <div class="main">
          <div class="slideshow">
            ...
            <!-- Image slot -->
            <img id="playlist-image" class="slide" alt="slide" />
            <!-- Video slot -->
            <div id="video-container"><video id="video-player" autoplay muted playsinline controls preload="auto"></video></div>

            <div class="watermark"><img src="/images/{{ logo }}" alt="Logo"></div>
            <div class="label-chip" id="labelChip">1 / {{ media_all|length }}</div>
            <div class="nav-buttons"><button id="prevBtn" type="button">⟵ Back</button><button id="nextBtn" type="button">Next ⟶</button></div>
          </div>

          <div class="info">
            <div class="logo"><img src="/images/{{ logo }}" alt="Logo"></div>
            <h1 id="person-name">Hi, I am Abigail Antwi</h1>
            <div id="person-message">
              <p>Welcome to the <strong>Divine Brain Lottery Forecast Centre</strong>.</p>
              <p><em>Don’t give up. Join the family that cares about your future. At Divine Brain Lottery forecast centre, your miracle could be just one prediction away.</em></p>
            </div>
...
<a href="{{ url_for('disclaimer', next=url_for('login')) }}" class="app-link">Visit Our App</a>
          </div>
        </div>

        <div class="bottom-bar">
          <div class="credits-track"><span class="credits-text" id="credits">A very big thank you to our sponsors: Mr Kaspa Antonio Afriyie, Mr Emmanuel Kwame Dua an Engineer and a building contractor at Metcam Gh limited. Mawusi P. Kpegla of Revolution Ventures, Abigail Antwi of Kesty Queens Academy, Mr Noah Adai, Samuel Odame Alema Junior of 'Odame World Films production And ALEMA FARMS. God bless you all...</span></div>
          <div id="disclaimer" class="disclaimer hidden">⚠️ 18+ only. Disclaimer: We don’t encourage you to become a  gambler — we rather teach you to gamble safe.</div>
        </div>
      </div>

<script>
  // Online stats refresh
  async function refreshStats(){
    try{
      const res = await fetch("/stats/online", {cache:"no-store"});
      if(!res.ok) return;
  ...
  // Playlist (images timed, videos wait for 'ended')
  const media = {{ media_all|tojson }};
  const captionsMap = {{ captions|tojson }};
  ...
  let idx = 0, auto = true, manualTimer = null, MANUAL_TIMEOUT = 30000;

  function updateLabel(){
    if(media.length===0){ labelChip.textContent='No media'; return; }
    labelChip.textContent=(idx+1)+' / '+media.length;
  }

  // --- Owner detection
  function ownerOf(name){ return name.toLowerCase().startsWith('cxk') ? 'christiana' : 'abigail'; }

  // --- Typing control: ONLY Christiana types; cancel on switch to Abigail
  let typeToken = 0;                       // global cancel token
  function cancelTyping(){ typeToken++; }  // bumping this cancels any in-flight type
  ...
  function typeText(el,text,speed=18){
    ...
    return new Promise(res=>{
      ...
      (function t(){
        if (myToken !== typeToken) return res(); // aborted
        if(i<text.length){
          el.innerHTML=text.slice(0,i+1)+'<span class="typewriter"></span>';
  ...
  async function setSide(owner, words){
    if(owner==='christiana'){
      personName.textContent='Christiana Xoryesuse Kove';
      contacts.style.display='none';
      await typeText(personMessage, words, 16); // typing only for Christiana
    } else {
      personName.textContent='Hi, I am Abigail Antwi';
      contacts.style.display='block';
      cancelTyping();                            // stop any Christiana typing
      personMessage.innerHTML='<p>'+words+'</p>';
    }
  }
  ...
  function hideAll(){
    ...
    try{
      ...
      videoPlayer.removeAttribute('data-name');
      videoPlayer.load();
    }catch(e){}
  }
  ...
  function showVideo(name){
    // If the same video is already playing, don't restart it
    if (videoContainer.style.display === 'flex' &&
        videoPlayer.getAttribute('data-name') === name &&
        !videoPlayer.ended && !videoPlayer.paused) {
      return;
    }
    hideAll();
    videoContainer.style.display='flex';
    videoPlayer.setAttribute('data-name', name);
    videoPlayer.src='/images/'+name;
    const p = videoPlayer.play();
    if (p && p.catch) { p.catch(()=>{}); }
  }

  // Safer auto-resume that never cuts a video short
  function autoResume(){
    if(auto) return;
    const m = media[idx];
    ...
    if (videoStillPlaying) {
      manualTimer = setTimeout(autoResume, 5000);
      return;
    }
    auto = true;
    runLoop();
  ...
  function enterManual(){
    ...
    manualTimer = setTimeout(autoResume, MANUAL_TIMEOUT);
  }

  function prev(){ enterManual(); idx=(idx-1+media.length)%media.length; renderCurrent(); }
  function next(){ enterManual(); idx=(idx+1)%media.length; renderCurrent(); }
  prevBtn.addEventListener('click', prev);
  nextBtn.addEventListener('click', next);
  document.addEventListener('keydown', e=>{ if(e.key==='ArrowLeft') prev(); if(e.key==='ArrowRight') next(); });

  // Manual triggers only from real gestures or pause
  videoPlayer.addEventListener('pause', enterManual);
  ['click','pointerdown','touchstart'].forEach(evt=>{
    videoPlayer.addEventListener(evt, enterManual, {passive:true});
    videoContainer.addEventListener(evt, enterManual, {passive:true});
  });
  ...
  async function renderCurrent(){
    ...
    updateLabel();
    const m = media[idx];
    const words = captionsMap[m.name] || (ownerOf(m.name)==='christiana'
      ? "{{ (christi_greeting + ' ' + christi_body) | e }}"
      : "{{ default_abigail | e }}");
    await setSide(ownerOf(m.name), words);
    if(m.kind==='image') showImage(m.name); else showVideo(m.name);
  }

  // Natural end for videos
  ...
  async function playVideosIfExist(){
    const videosInPlaylist = media.filter(m => m.kind==='video').map(m => m.name);
    if(videosInPlaylist.length===0) return;
    videoContainer.style.display='flex';
    for(const v of videosInPlaylist){
      const owner = ownerOf(v);
      const words = captionsMap[v] || (owner==='christiana'
        ? "{{ (christi_greeting + ' ' + christi_body) | e }}"
        : "{{ default_abigail | e }}");
      await setSide(owner, words);            // ensures correct owner text
      videoPlayer.setAttribute('data-name', v);
      videoPlayer.src="/images/"+v;
      tryAutoplay(videoPlayer);
      await new Promise(res => videoPlayer.onended = () => res());
    }
    videoContainer.style.display='none';
  }
  ...
  async function runLoop(){
    while(auto){
      ...
      const m = media[idx];
      if(m.kind==='image'){
        await new Promise(r=> setTimeout(r, IMAGE_DURATION));
      } else {
        await new Promise(r=>{
          const onEnd = ()=>{ videoPlayer.removeEventListener('ended', onEnd); r(); };
          videoPlayer.addEventListener('ended', onEnd, {once:true});
        });
      }
  ...
  renderCurrent().then(()=> runLoop());

  // Falling numbers + credits/disclaimer swap
  const numbersLayer=document.getElementById('numbers'); let currentNumber=90;
  function spawnNumber(){
    const d=document.createElement('div'); d.className='number'; d.textContent=currentNumber;
    currentNumber=(currentNumber>1)?currentNumber-1:90; d.style.left=Math.random()*98+'%';
    const sx=10+Math.random()*25, fd=12+Math.random()*10, sd=3+Math.random()*3;
    d.style.setProperty('--sx',sx+'px'); d.style.setProperty('--dur',fd+'s'); d.style.setProperty('--swayDur',sd+'s');
    d.style.fontSize=(20+Math.random()*16)+'px'; d.style.opacity=(0.55+Math.random()*0.35).toFixed(2);
    numbersLayer.appendChild(d);
    if(numbersLayer.childElementCount>120) numbersLayer.removeChild(numbersLayer.firstElementChild);
    setTimeout(()=>d.remove(),(fd+0.2)*1000);
  }
  setInterval(spawnNumber,280);
  const credits=document.getElementById('credits'), disclaimer=document.getElementById('disclaimer');
  credits.addEventListener('animationiteration',()=>{
    credits.classList.add('hidden'); disclaimer.classList.remove('hidden');
    setTimeout(()=>{ disclaimer.classList.add('hidden'); credits.classList.remove('hidden'); },12000);
  });
</script>
    ...
    return render_template_string(
        ...
        media_all=media_all,
        captions=captions,
        christi_greeting=christi_greeting,
        christi_body=christi_body,
        default_abigail=default_abigail,
        online_now=online_now,
        total_visits=TOTAL_VISITS,
        logo=LOGO_NAME
    )

# Serve the media strictly from Abii
...
def disclaimer():
    next_url = request.args.get("next") or request.form.get("next") or url_for("login")
    ...
    <head>
      <meta charset="utf-8">
      <title>Disclaimer & Responsible Use — DBLFC</title>
      ...
      <style>
        ...
        /* Background image (engine.jpg in Abii folder) */
        body::before{
          content:""; position:fixed; inset:0;
          background: url('/images/engine.jpg') center/cover no-repeat fixed;
        ...
        body::after{
          content:""; position:fixed; inset:0;
          background: radial-gradient(ellipse at 30% 10%, rgba(212,175,55,.15), transparent 45%),
                      linear-gradient(to bottom, rgba(0,0,0,.25), rgba(0,0,0,.55));
          z-index:-1;
        }

        /* Card now uses flex so the middle scrolls if long */
        .card{
          width:min(980px, 92vw);
          max-height: 92vh;             /* <- keep card on screen */
          margin:36px 16px;
        ...
        .hdr img{ width:42px; height:42px; border-radius:10px; box-shadow:0 2px 10px rgba(0,0,0,.4) }
        .hdr h1{ margin:0; font-size:1.25rem; letter-spacing:.3px; color:var(--gold) }

        .body{
          flex:1 1 auto;                 /* <- grow and scroll here */
          padding:24px; line-height:1.55;
          overflow:auto;
          -webkit-overflow-scrolling: touch;
        }
        .body h2{ margin:18px 0 8px; color:var(--gold); font-size:1.1rem }
        .dim{ color:#e9e9e9; opacity:.95 }
        .body p{ margin:10px 0 }

        .foot{
          flex:0 0 auto;
          padding:16px 22px; display:flex; gap:12px; align-items:center; justify-content:flex-end;
        ...
        .btn{
          ...
          border-radius:10px; color:#000; background:var(--gold);
        ...
        .link{
          ...
          border:1px solid rgba(255,255,255,.25); border-radius:10px; background:transparent;
        }
        .link:hover{ background:rgba(255,255,255,.08) }
        .age{ margin-right:auto; display:flex; gap:10px; align-items:center; font-size:.95rem; opacity:.95 }
        .age input{ transform: scale(1.2) }
    ...
    <body>
      <form class="card" method="post">
        <div class="hdr">
          <img src="/images/db_lotto_hall.jpg" alt="Logo">
        ...
        <div class="foot">
          <label class="age">
            <input id="ageok" type="checkbox" required>
            I confirm I am 18+ and I accept the disclaimer.
          </label>
          <input type="hidden" name="next" value="{{ next_url }}">
          <a class="link" href="/">Back</a>
          <button id="agreeBtn" class="btn" type="submit" disabled>I Agree — Continue</button>
        </div>
      </form>

      <script>
        // Enable button only when checkbox is ticked
        const cb = document.getElementById('ageok');
...
app.config['SECRET_KEY'] = 'lotto-forecast-2025-keep-this-secret'
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///lotto_users.db'
...
# --- BEGIN: Admin import-now endpoint (PA/PC safe) ---
import os
from flask import request, redirect, url_for, flash, abort, jsonify

# Ensure these two directories exist on both PC and PA.
# If you already defined UPLOAD_DIR / RESULTS_DIR elsewhere, we reuse them.
if 'UPLOAD_DIR' not in globals() or 'RESULTS_DIR' not in globals():
    if os.environ.get('PYTHONANYWHERE_DOMAIN'):
        BASE_DIR    = "/home/kakra4065"
        UPLOAD_DIR  = os.path.join(BASE_DIR, "data", "uploads")
        RESULTS_DIR = os.path.join(BASE_DIR, "data", "predictions")
    else:
        # Local PC defaults (adjust these to your real local folders if different)
...
# Small helper to hash files (reused if it's already in the file)
if '_sha256_file' not in globals():
    import hashlib
...
# If you already have PredictionFile model, we use it.
# If not, we degrade gracefully to a simple JSON answer.
PredictionFileModel = globals().get('PredictionFile', None)

ADMIN_IMPORT_TOKEN = os.environ.get("ADMIN_IMPORT_TOKEN")  # set on PA (Web → Env vars)
# For local testing without PA env, you may allow missing token:
ALLOW_LOCAL_IMPORT = not os.environ.get('PYTHONANYWHERE_DOMAIN')

def _import_scan_once():
    """
    Scan uploads/results folders, create/update PredictionFile rows if the model exists.
    Matches by base name: foo.txt (input) and foo.txt.pred.txt OR foo.json (result).
    """
    # Index results by base (strip .pred.txt or .json)
    result_index = {}
    for fname in os.listdir(RESULTS_DIR):
        lf = fname.lower()
        if lf.endswith(".pred.txt") or lf.endswith(".json"):
            if lf.endswith(".pred.txt"):
                basekey = fname[:-len(".pred.txt")]
            else:
                basekey = os.path.splitext(fname)[0]
            result_index[basekey] = fname
    ...
    for fname in os.listdir(UPLOAD_DIR):
        lf = fname.lower()
        if not (lf.endswith(".txt") or lf.endswith(".json")):
            continue
        ...
        basekey = os.path.splitext(fname)[0]
        result_name = result_index.get(basekey)
        seen += 1

        if PredictionFileModel is None:
            # No model — just report what we would do.
            continue
        ...
        pf = PredictionFileModel.query.filter_by(sha256=sha or "").first() if sha else None
        if not pf:
            pf = PredictionFileModel(
                original_name=fname,
                stored_name=fname,
                sha256=sha or "",
            ...
            pf.stored_name = fname
            pf.size_bytes  = os.path.getsize(input_path)
            pf.status      = pf.status or "processing"
            pf.error_msg   = None
            updated += 1

        if result_name:
            pf.result_name = result_name
            pf.status = "done"

        db.session.commit()

    return {"inputs_seen": seen, "imported": imported, "updated": updated, "results_seen": len(result_index)}

@app.get("/admin/import-now")
def admin_import_now():
    # simple token for PA; allow no-token on local dev for convenience
    token = request.args.get("api_key") or request.headers.get("X-API-Key")
    if not ALLOW_LOCAL_IMPORT:
        if not ADMIN_IMPORT_TOKEN or token != ADMIN_IMPORT_TOKEN:
            abort(403)

    res = _import_scan_once()
    # If you have an admin index endpoint, redirect with a flash; else return JSON:
    try:
        flash(f"Imported: {res}", "success")
        return redirect(url_for("admin"))
    except Exception:
        return jsonify(res), 200
# --- END: Admin import-now endpoint (PA/PC safe) ---
...
def logout():
    ...
    return redirect(url_for('alema_home'))
...
def admin():
    ...
    online_now = lambda u: (
        u.last_active and
        (((datetime.now(UTC) - (u.last_active.replace(tzinfo=UTC) if u.last_active.tzinfo is None else u.last_active)))) < timedelta(minutes=10)
    )
...
def admin_adverts():
    ...
    upload_folder = os.path.join('static', 'images.advirt')
...
def main():
    ...
    import os
    ad_folder = os.path.join('static', 'images.advirt')
    ad_files = [
        f for f in os.listdir(ad_folder)
        if f.lower().endswith(('.jpg', '.jpeg', '.png', '.gif', '.mp4', '.webm', '.ogg'))
    ]
